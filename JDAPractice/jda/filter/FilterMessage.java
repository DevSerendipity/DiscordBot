...
public class FilterMessage extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        if (bot.filters.FilterOnOff.filterOn) {
            if (e.getMessage().getContentRaw().equalsIgnoreCase(BotCommandCenter.prefix + "filtermessage") && !allowed) {
                e.getChannel().sendMessage("Filter Response Has Been Enabled.").queue();
                System.out.println("Enabled");
                allowed = true;
            } else if (e.getMessage().getContentRaw().equalsIgnoreCase(BotCommandCenter.prefix + "filtermessage") && allowed) {
                e.getChannel().sendMessage("Filter Response Has Been Disabled.").queue();
                System.out.println("Disabled");
                allowed = false;
            }
        } else if (e.getMessage().getContentRaw().equalsIgnoreCase(BotCommandCenter.prefix + "filtermessage") && !FilterOnOff.filterOn) {
